...
- In a nutshell, we show how to efficiently create a chain of our preferred tools to tackle a map visualisation part of a ficticious PhD
- We try to go 'end-to-end' and consider Research Reproducibility/we anticipate an expert questioning our results and wanting to reproduce our results a few years after publication
- Be aware: We use a geospatial example, but the tools and workflows are applicable to many research disciplines
...
- Example = End-to-End workflow with data
  - Depth of covered aspect varies; also depends on your research
...
- **our project** is part of a PhD project (usually 3-5 years runtime), how much of the shown workflows apply to your work differs
- from this 1 hour up to 3 years (or more?) and you can mix-and-match components
...
::: {.callout-note}
because of the way we interact with the core-coding task, we don't see too much of the OS
:::
...
- if availabile try one of these sessions
- My summary: If your goal is...
...
- we can get a lot done by only using a command-line interface (aka CLI/terminal/console/shell/BaSH/ZSH/Fish/...)
- open locally or log in to a VM (via [SSH](https://en.wikipedia.org/wiki/Secure_Shell), etc.)
...
TODO #26 VIDEO HOW WE LOG IN VIA SSH. HOW WE CREATE A .PY FILE AND EXECUTE IT
...
## LIVE DEMO/VIDEO 
Let's try to write another hello-wordl, just like we did on the CLI. Cmd-N, save, file name, .py extension. Go to Terminal. Execute it `python helloworld.py`
...
## Live Demo: Let's create a new Jupyter Notebook, write a hello-world, run it, save it, and push it to GitHub
...
- `if` **all** stakeholders agree (Research Data Policy, Ethics Commission, IP Advisor, Funder,...) 
  - we can host our code on **version controlled repository**
- track changes (what changed from previous to now, what from now to 23 days ago)
- we don't want file names such as `Thesis final final really final 2 july.docx` 
- code is mostly plain text (not `.docx`,...), we can use thigs like a `diff`
...
- (This topic is not crucial, I know people who use it frequently and other that are successful and don't do it at all)
- What is this `venv`? This is one way (there are others) of creating a virtual environment. 
...
- Some consider it good practice to keep one `venv` per project. If you research and use of Python on one specific machine is just about one project, this might be ignorable; `poetry` spins up a virtual environment anyways for you
- overall, this kind of encapsulation can support Reproducibility

VIDEO here I install everything with `pip install requirements.txt`
...
- Containers (Think of a mini-VM) [Docker](https://www.docker.com/), [Kubernetes](https://kubernetes.io/)
    	- Pick/reference an OS (say Ubuntu 22.4) + a Python version (say 3.11.6), etc. your local/VM machine runs Kernel and VSC
    	- put the result on [DockerHub](https://hub.docker.com/)
- [BinderHub](https://research-hub.auckland.ac.nz/research-software-and-computing/advanced-compute/nectar-binderhub): package a remote Git repository into an interactive Jupyter notebook for displaying code and output
- [Research Object](https://www.researchobject.org/ro-crate/) (RO) Crate: an approach to package research data and its metadata human and machine readable, [Workflow](https://about.workflowhub.eu/Workflow-RO-Crate/) hub can ingest or generate an RO-Crate that describes the complete processes, authorship, and requirements of a computational workflow
...
| IDE                | `VSC` to run a Jupyter Notebook (ssh'd to VM)          |
| Code               | On `GitHub`                                            |
...
- (VM is created, VSC is installed, plugins are installed, python-is-python3)
- Open VSC
- ssh
- create venv
- create ipynb (maybe touch via console or left panel)
- (copy-)paste in md headings/skeleton structure
- push run button
- select kernel
- (not sure: create new gh repo, make private)
- (not sure where/how: magically we know all needed libs; pip freeze it)
- show commit in VSC extension
- navigate to website and see what we did

## Background: What we plot
- StatsNZ decomposes New Zealand into Statistical Areas 
	- each SA should encapsule people of similar socio-economical background
	- there are 3 resolutions (SA1 = x people, SA2 = y people, SA3 = z people)
	- we pick SA2

- we want to plot the 
	- boundaries (polygon on the map)
	- name of each SA2
	- number of people 
and show this on an interactive map in a browser
